...
status: 三项已决定（2026-09-20）：Web 引入 React，主库 Ant Design 6 加 Ant Design X；小程序主库 tdesign-miniprogram；飞书感不作硬要求。选用清单与桥接产物仍是建议，接入要逐项授权
updated: 2026-09-20
---
...
> 2026-09-20 已决定：Web 端引入 React，主库用 Ant Design 6 加 Ant Design X；小程序主库用 tdesign-miniprogram；飞书感不作硬要求。下面的路线 A 与 Semi 保留为备选记录。
...
### Web：岛屿式 React 加 Ant Design 6，先做样板（已决定）

任务单见 `07-接入计划.md`。

1. 需授权立项。用 Vite 加 React 18，装 antd 6 与 @ant-design/x 2.9。在任务工作台右侧详情容器里 `createRoot` 挂载。
2. 用 `<ConfigProvider theme={bridge}>` 引入 `dist/bridge-antd.theme.json`，同时引 `dist/design-tokens.css`。antd 从 colorPrimary 自行派生色阶。
3. 样板要量清楚首屏 JS 体积、与现有弹层和 z-index 的冲突、返回列表是否保留筛选与位置。
4. AI 界面按 A 系列原则用 Ant Design X 组件做一处，比如即时总结的来源引用。
...
- D-4 主色十档：Web 端 antd 自行派生，不用我们的十档。小程序端按 Ant Design 算法派生，有网络的同事用 TDesign 主题生成器复核后决定是否写回 `tokens.json`。
...
- 三项选型已决定（2026-09-20）。选用清单、`bridges.json` 与桥接产物仍是建议，接入并验收后才登记采用。
