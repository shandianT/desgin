...
(function(global){
  ...
  function open(config){
    ...
    const state={dialog,anchor:config.anchor,owner:config.owner,instances:[],onClose:config.onClose};active=state;
    const valid=()=>active===state&&live(config.owner)&&(!config.anchor||config.anchor.isConnected);
    ...
    picker=new TomSelect(input,{options:normalize(config.options||[]),items:(config.selected||[]).map(String),maxItems:config.multiple?(config.maxItems||null):1,
      ...
      render:{no_results:()=>'<div class="no-results">暂无匹配结果</div>',loading:()=>'<div class="spinner">正在加载…</div>',
    ...
    update();position();requestAnimationFrame(()=>{if(valid()){picker.refreshOptions(false);picker.open();position();}});
  }
  function nativeSelect(select){
    ...
    open({anchor:select,title,description:listFilter?'仅筛选商机列表':summaryYear?'切换统计季度的年份，仅影响总览数字':'',multiple:select.multiple,
  ...
  function intercept(owner,name,data,native,detail={},type='tap'){
    ...
    const opts=rows=>(rows||[]).map(row=>({value:row.id??row.value??row.key,text:row.name??row.label??row.value,meta:[row.account_code,row.group,row.team].filter(Boolean).join(' ')}));
    ...
    if(tag==='dashboard-picker'&&name==='open'){
      ...
      open({...config,title:p.title,description:p.subtitle,multiple:p.multiple,minItems:1,options:opts(p.options),selected:p.selected,commit:ids=>trigger('change',{ids})});return true;
    }
    ...
    if(tag==='profile-scope-picker'&&name==='show'){
      ...
      open({...config,title:kind==='team'?'选择团队':'选择成员',options:opts(kind==='team'?p.teams:p.members),selected:[kind==='team'?p.teamId:p.memberId].filter(Boolean),commit:ids=>trigger('subjectchange',{kind,id:ids[0]})});return true;
    }
  ...
  function reconcile(){if(active&&(!live(active.owner)||(active.anchor&&!active.anchor.isConnected)))close();}
