...
npm i ./release/sensetime-dept-tokens-1.1.0-draft.1.tgz ./release/sensetime-dept-ui-react-0.2.0.tgz

# 小程序（在 frontend/miniprogram 下）
npm i tdesign-miniprogram@1.16.1 ./release/sensetime-dept-ui-miniprogram-0.2.0.tgz
```
...
发布顺序：先 tokens，再 ui-react 和 ui-miniprogram。每次发布前跑 `node tools/check.mjs` 和 `node tools/pack.mjs`，在采用登记表登记版本。
