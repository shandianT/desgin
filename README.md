...
└── specs/salesbuddy/         SalesBuddy 规范：README、00～04 章、02 变量与同步链路、03 跨端样板、采用登记表、依据、1.0.0 原件、rules.json、规则索引.md
```
...
| AI 技能与钩子 | 已建立；触发评测待做 |
| 可视化规范站 | 未做（下一步候选，见 `specs/salesbuddy/00-本轮工作说明-20260919.md`） |
| 决定 | 主色沿用 #2863CD；本仓库为共同入口；原生 App 暂不在范围 |

## 怎么用

- **看规范**：`specs/salesbuddy/README.md`，或直接看 `specs/salesbuddy/规则索引.md`（38 条一张表）。
- **看效果**：浏览器打开 `specs/salesbuddy/03-跨端样板-客户列表到详情/index.html`（拖窗口看三档）或 `对照.html`（三端并排）。
