 ...
 */
export function SbTable({ columns = [], rows = [], rowKey = 'id', state = 'normal', emptyTitle, emptyDescription, onRetry, onClear, actions, actionsWidth = 120, pagination, expandable, density = 'default', onRowClick, scrollX, className = '' }) {
  ...
  return (
    <div className={`sb-table sb-table-${density} ${className}`}>
      <Table columns={cols} dataSource={panel ? [] : rows} rowKey={rowKey} loading={state === 'loading'} pagination={false} size={density === 'compact' ? 'small' : 'middle'} expandable={expandable} scroll={scrollX ? { x: scrollX } : undefined} locale={{ emptyText: panel || ' ' }} onRow={onRowClick ? (row) => ({ onClick: () => onRowClick(row) }) : undefined} />
