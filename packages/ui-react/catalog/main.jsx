...
function LabeledSelectDemo() { const [multi, setMulti] = useState([]); const [q, setQ] = useState(); const [p, setP] = useState('q3'); const opts = [{ value: 'attack', label: '主攻区', count: 9 }, { value: 'asset', label: '客户资产', count: 12 }, { value: 'spot', label: '见单打单', count: 3 }, { value: 'resource', label: '客户资源', count: 7 }]; return <div className="states"><State title="未选：显示全部"><SbLabeledSelect label="象限" value={q} onChange={setQ} options={opts} /></State><State title="已选，可清空回到全部"><SbLabeledSelect label="周期" value={p} onChange={setP} options={[{ value: 'q3', label: '第三季度' }, { value: 'q4', label: '第四季度' }]} /></State><State title="复选框：勾选后应用；取消保持原条件"><SbLabeledSelect label="象限" mode="multiple" value={multi} onChange={setMulti} options={opts} /><p>已应用：{multi.join("、") || "全部"}</p></State><State title="多个并排"><div className="inline"><SbLabeledSelect label="象限" value={q} onChange={setQ} options={opts} /><SbLabeledSelect label="计划" options={[{ value: 'y', label: '有 IT 计划' }, { value: 'n', label: '无' }]} /><SbLabeledSelect label="金额" disabled options={[]} /></div></State></div>; }
