...
function LabeledSelectDemo() { const [q, setQ] = useState(); const [p, setP] = useState('q3'); const opts = [{ value: 'attack', label: '主攻区', count: 9 }, { value: 'asset', label: '客户资产', count: 12 }, { value: 'spot', label: '见单打单', count: 3 }, { value: 'resource', label: '客户资源', count: 7 }]; return <div className="states"><State title="未选：显示全部"><SbLabeledSelect label="象限" value={q} onChange={setQ} options={opts} /></State><State title="已选，可清空回到全部"><SbLabeledSelect label="周期" value={p} onChange={setP} options={[{ value: 'q3', label: '第三季度' }, { value: 'q4', label: '第四季度' }]} /></State><State title="多个并排"><div className="inline"><SbLabeledSelect label="象限" value={q} onChange={setQ} options={opts} /><SbLabeledSelect label="计划" options={[{ value: 'y', label: '有 IT 计划' }, { value: 'n', label: '无' }]} /><SbLabeledSelect label="金额" disabled options={[]} /></div></State></div>; }
function MetricStripDemo() { const [period, setPeriod] = useState('year'); const items = [{ key: 'acv', label: '年度合同额 · 万元', value: period === 'year' ? 1880 : 5260 }, { key: 'rev', label: '确收 · 万元', value: period === 'year' ? 138 : 412 }, { key: 'cash', label: '回款 · 万元', value: null, note: '财务还没登记' }, { key: 'cnt', label: '客户', value: 24, note: '比上季 +2 家' }]; return <div className="states wide"><State title="带周期与口径"><SbMetricStrip items={items} periods={[{ value: 'year', label: '本年' }, { value: 'all', label: '历年' }]} period={period} onPeriodChange={setPeriod} caliber="确收按合同签署月，回款按到账月，缺失显示未登记" /></State><State title="加载中：不显示 0"><SbMetricStrip items={items.slice(0, 3)} loading /></State></div>; }
