...
- `SbLabeledSelect` 的多选默认使用复选框确认：连续勾选后点击应用才调用 `onChange`；取消、Esc 和外部点击保留原条件，支持搜索与键盘。单选不变；既有使用方若需要即时多选可显式传 `confirmMultiple={false}`。
- `SbBattleMap` 新增可选 `layout="equal"`：按阈值分段映射为四个等大区域，保持原始评分和分类；默认仍为线性布局。
- `SbBattleMap` 按绘图区实际宽高更新 SVG，支持客户页横向加宽；默认正方形样式保留，圆点、文字、提示与放大操作随容器正确定位。
- Web 全部 7 处多选筛选统一采用该交互，包括客户优先级／协助成员、销售阶段、看板阶段及 FDE 人员／阶段／关单季度。优先级、看板阶段及 FDE 阶段／季度均一次更新条件后查询。
- `SbBattleMap` 客户点一律主色实心，不再按 tone 上黄、红或画空心点（2026-09-24 决定，见 12 章 2.3）；只用点径表示金额档；状态写进悬停提示（以前传了 summary 就不显示状态，现在两者都显示）与点的读屏文字。`tone` 属性照收，不再影响颜色，使用方不用改代码。
