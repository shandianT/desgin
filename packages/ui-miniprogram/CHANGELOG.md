...
- 小程序状态色学飞书（tokens 1.5.0-draft.1，只改小程序端）：状态文字用飞书同色系深一档（红 #AC2F28、橙 #8F4F04、绿 #237B19，浅底上都 ≥4.5:1），浅底用飞书 1 档，色条用 5 档鲜色（红 #F54A45、橙 #FF8800）。提醒从黄棕改为飞书的橙。
- sb-summary-card：tone=bad 的色条改用新变量 `--ui-danger-accent`（小程序为飞书鲜红，Web 与 --ui-danger 同色）。
- sb-state-panel：加载态转圈与文字分开排，文字占满一行（以前放在 t-loading 里横排，长提示被挤成竖排三行）。
- sb-tabs：窄于 360px 退回横滑时右边缘渐隐，提示还能滑，不再露出被切断的半个页签；当前页签自动滚进可见区。
- 测试：36 个组件、146 个状态。

## 0.10.0（2026-09-24）

- sb-summary-card 新增 `tone`：`bad` 顶部 3px 红条、`watch` 橙条（tokens 1.4.0-draft.1 新增 `--ui-warning-accent`）、`brand` 主色条；`good`、`pending` 与不传都不画。用户在「上沿浅色渐变」与「顶部色条」两套整页样板里选了色条（2026-09-24）。只给对象详情页顶卡在状态异常时、看板主卡用；颜色和同卡状态标签一致，状态仍以标签为准。规则见 16 章第 11 条。
- 测试：36 个组件、146 个状态。

## 0.9.1（2026-09-24）

- sb-battle-map：客户点一律主色实心，不再按 tone 上黄、红或画空心点（2026-09-24 决定，见 12 章 2.3）；只用点径表示金额档，状态在 pointtap 后的客户卡里看。重叠聚合的圆也改主色（以前是次要灰），和 Web 端一致。`tone` 照收，点上保留 `tone-*` 类名给页面做钩子，组件不再按它上色。
