...
仓库原来只有打包产物。本次恢复 43 个展示源文件；其中 36 个保持原样，7 个作上述修改。来源 ZIP 中三份产物与基线逐字节相同，归档、产物及原始源文件的 SHA-256 见 [来源记录](source-provenance.json)。业务 `demo/web/bundle.js` 没有修改。
...
| `node tools/verify-multiple-filters.mjs` | 新增 6/6 组通过；连同原 13 组，合计 19 组通过。覆盖另外 6 处多选的暂存、取消、一次应用、清空，优先级结果和分页、阶段结果、季度结果，以及原单选保持。 |
| 重建一致性 | 最终 Web 构建连续两次得到相同缓存标识 `dabedfbfb49a`。 |
