(function (global) {
  ...
  function stageFields(fields, ctx) {
    if (!fields || FIELD_KEYS.some(key => !(key in fields)) || Object.keys(fields).some(key => !FIELD_KEYS.includes(key))) ctx.error(422, '示例质检须提交完整的规范字段快照');
    for (const key of FIELD_KEYS) if (typeof fields[key] !== (key === 'is_first_visit' ? 'boolean' : 'string')) ctx.error(422, '示例质检字段类型不正确：' + key);
  ...
  function assertFdeTarget(ctx, customerId, opportunityId) {
    ...
    if (!opportunityId) ctx.error(422, '示例 FDE 须选择本人参与的商机');
    const row = ctx.opportunity(opportunityId);
    if (row.customer_id !== customerId || !(row.fde_member_ids || []).includes(ctx.actor.user_id)) ctx.error(403, '该示例商机不属于当前客户或本人未参与，不能录入');
  }
  function qualifiedReview(ctx, fields) {
    const run = ctx.state.runs[fields._quality_review_run_id];
    if (!run || run.actor_id !== ctx.actor.user_id || !['visit_entry', 'visit_quality'].includes(run.mode) || run.status !== 'succeeded') ctx.error(422, '请先完成当前身份的示例字段校验');
    if (run.mode === 'visit_quality') {
      ...
      if (canonical(expected) !== canonical(archiveSnapshot(ctx.body))) ctx.error(409, '拜访字段、关联对象或参与人已修改，请重新执行示例字段校验');
      stageFields(fields && Object.fromEntries(FIELD_KEYS.map(key => [key, fields[key]])), ctx);
    }
    if (run.customer_id !== ctx.body.customer_id || isFde(ctx.actor) && run.opportunity_id !== fields.opportunity_id) ctx.error(409, '关联对象已变化，请重新执行示例字段校验');
    if (run.review_text !== reviewContent(fields)) ctx.error(409, '拜访正文已修改，请重新执行示例字段校验');
    const quality = checkFields(fields);
    if (quality.follow_up_score !== 100 || !quality.next_action.passed) ctx.error(422, '示例字段校验未通过，请补齐正文和带日期的下一步行动');
    if (run.archived_visit_id) ctx.error(409, '这次示例校验已经归档；新增拜访请重新校验');
    for (const [key, label] of [['interaction_at', '跟进日期'], ['created_date', '创建时间'], ['contact_name', '对接人']]) if (!present(fields[key])) ctx.error(422, `请补充${label}`);
  ...
  function handle(ctx) {
    ...
    if (path === '/visit-flow/structure' && method === 'POST') {
      ...
      if (body.source_import_id) ctx.error(501, '示例模式不提供文件导入，请使用文字录入');
    ...
    if (path === '/visit-flow/quality' && method === 'POST') {
      ...
      if (isFde(actor) && (body.opportunity_mutation || participants.length || collaborators.length || body.fields.is_first_visit)) ctx.error(403, '示例 FDE 本人录入不能修改商机、首次拜访或代选参与人');
    ...
    if ((match = path.match(/^\/conversations\/([^/]+)\/messages$/)) && method === 'POST') {
      ...
      if (conversation.actor_id !== actor.user_id) ctx.error(404, '当前身份不可访问该示例会话');
      ...
      if (conversation.mode === 'visit_entry') {
        ...
        result = {opportunity_draft: parsed, source_label: '示例标签提取 · 只保留原文明确填写的商机字段', demo: true};
      }
    ...
    if (path === '/visits' && method === 'POST') {
      ...
      if (isFde(actor) && (fields._opportunity_mutation || (body.fde_participant_ids || []).length)) ctx.error(403, '示例 FDE 本人录入不能修改商机或代选其他参与人');
      ...
      if (participantIds.some(id => !actors.some(person => person.user_id === id && isFde(person)))) ctx.error(422, '请重新核对实际参与的示例 FDE');
      if (fields._opportunity_mutation) {
        if (typeof ctx.saveOpportunity !== 'function') ctx.error(501, '示例商机与拜访共同保存尚未接入');
    ...
    if ((match = path.match(/^\/visits\/([^/]+)$/)) && method === 'PATCH') {
      ...
      if (!row) ctx.error(404, '示例拜访不存在');
      ctx.customer(row.customer_id);
      if (isFde(actor) && row.recorder_id !== actor.user_id) ctx.error(403, '示例 FDE 只能补充本人记录');
      if (Number(body.version_no) !== row.version_no) ctx.error(409, '示例记录已变化，请刷新后重试');
