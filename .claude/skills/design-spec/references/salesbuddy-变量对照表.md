...
|---|---|---|---|---|---|---|---|
| `--ui-background` | 工作区背景 | `color.gray.50` | #F3F5F9 | #F3F5F9 | **#F5F6F7** | 已确认（电脑值）；端侧覆盖为建议 | V-01 |
| `--ui-surface` | 内容面板 | `color.gray.0` | #FFFFFF | #FFFFFF | #FFFFFF | 已确认 | V-01 |
| `--ui-sidebar` | 深蓝导航 | `color.navy.900` | #142F54 | #142F54 | #142F54 | 已确认 | V-01 |
| `--ui-ink` | 正文文字 | `color.ink.900` | #192842 | #192842 | **#1F2329** | 已确认（电脑值）；端侧覆盖为建议 | V-01 |
| `--ui-secondary` | 次要文字 | `color.ink.600` | #5B6C82 | #5B6C82 | **#646A73** | 已确认（电脑值）；端侧覆盖为建议 | V-01 |
| `--ui-muted` | 辅助文字 | `color.ink.500` | #617188 | #617188 | **#646A73** | 已确认（电脑值）；端侧覆盖为建议 | V-01 |
...
| `--ui-text-page` | 页面标题 | `font.size.24` | 24px | 24px | **20px** | 已确认（电脑值）；端侧覆盖为建议 | V-02 |
...
| `--ui-radius-panel` | 面板圆角 | `radius.12` | 12px | 12px | **8px** | 已确认（电脑值）；端侧覆盖为建议 | V-03 |
| `--ui-control-height` | 普通控件高 | `size.36` | 36px | 36px | 36px | 已确认 | V-03 |
| `--ui-field-height` | 表单控件高 | `size.40` | 40px | 40px | **44px** | 已确认（电脑值）；端侧覆盖为建议 | V-03 |
...
| `--ui-shadow-popup` | 弹出层阴影（当前为 CSS 字符串形式，Style Dictionary 可直接输出；待工具链完整支持 2025.10 后迁移为对象形式） | 直接值 | 0 8px 28px #1928421a, 0 2px 6px #19284208 | 0 8px 28px #1928421a, 0 2px 6px #19284208 | **0 0 1px #0000004d, 0 4px 14px #0000001a** | 已确认（电脑值）；端侧覆盖为建议 | C-07 |
...
| `--ui-overlay` | 弹窗与抽屉背后的遮罩（深蓝导航色 45% 透明） | 直接值 | #142F5473 | #142F5473 | **#16161A99** | 建议（电脑值）；端侧覆盖为建议 | C-07 |
...
- **微信小程序**：微信小程序：生成 WXSS，单位 px（逻辑像素）；宽度按比例缩放的部分由页面自行用 rpx。2026-09-23 起中性色、圆角、遮罩、页面标题按飞书 Universe Design 对齐（主色保留 #2863CD：飞书蓝 #3370FF 白底对比 4.28:1，不满足 D-06 的 4.5:1）（建议）
